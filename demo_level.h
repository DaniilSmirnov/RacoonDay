...
void open_demo_level(int Weidth,int Height){
    ...
    while (Level_Demo.isOpen())
    {
        ...
        for (int i = 0; i < map_height; i++)
            ...
            {
                if (demo_level_map[i][j] == ' ')  s_map.setTextureRect(IntRect(0,0,32,32)); //если встретили символ пробел, то рисуем 1й квадратик
                if (demo_level_map[i][j] == 'c')  s_map.setTextureRect(IntRect(32,0,32,32));//если встретили символ c, то рисуем 2й квадратик
                if (demo_level_map[i][j] == '0')  s_map.setTextureRect(IntRect(64,0,32,32));//если встретили символ 0, то рисуем 3й квадратик
                ...
                if (demo_level_map[i][j] == 'e') {
                    ...
                    if (enemies_counter != enemies) {
                        ...
                        enemy[enemies_counter].setPosition(Vector2f(Enemy_x[enemies_counter], Enemy_y[enemies_counter]));
                        enemies_counter++;
                    }
                }
        ...
        for (int i = Player_y / 32; i < (Player_y + 32) / 32; i++)//проходимся по тайликам, контактирующим с игроком, то есть по всем квадратикам размера 32*32, которые мы окрашивали в 9 уроке. про условия читайте ниже.
            ...
            {
                if (demo_level_map[i][j] == '0')//если наш квадратик соответствует символу 0 (стена), то проверяем "направление скорости" персонажа:
                {
